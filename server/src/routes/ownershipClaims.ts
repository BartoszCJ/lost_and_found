...
router.post('/', authenticateJWT, authorizeRole(['user']), createOwnershipClaim);

// Pobranie wszystkich zgłoszeń własności (dla pracowników/adminów)
router.get('/', authenticateJWT, authorizeRole(['employee', 'admin']), getOwnershipClaims);

// Aktualizacja statusu zgłoszenia (dla pracowników/adminów)
router.patch('/:id', authenticateJWT, authorizeRole(['employee', 'admin']), updateOwnershipClaim);
